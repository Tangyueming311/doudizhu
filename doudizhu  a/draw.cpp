...
void menu() {
	...
	gotoxy(MapLength / 2 - 8, MapHeight / 2 + 6);
	cout << "《《";
	...
	while (1) {
		...
		if (key == 119) {
			...
			if (key_point ==1) {
				...
				cout << "《《";
				gotoxy(MapLength / 2 + 10, MapHeight / 2 + 10);
				cout << "》》";
				key_point--;
				continue;
			}
		...
		//向下
			...
			if (key_point ==1) {
				//再打印箭头
				gotoxy(MapLength / 2 - 8, MapHeight / 2 + 14);
				cout << "《《";
				gotoxy(MapLength / 2 +10, MapHeight / 2 + 14);
				cout << "》》";
				//	temp_game = 0;
				key_point++;
				continue;
		...
		else if ((key == 100 ||key==97)&& (key_point == 0)) {
		...
		else if ((key == 100 || key == 97) && (key_point == 2)) {
...
int Choose_Landlord(int* handcard) {
	...
	gotoxy(MapLength / 2 - 8, tempLeft_y - 6);
	cout << "您的手牌如下：";
...
void buchu(int* a,int if_player_is_landlord) {
	...
	Draw(tempLeft_x, tempLeft_y - 4, (cardNum ) * 4 + 14, cardHeight+4);
	//Draw(0, 0, MapLength, MapHeight);
	if (if_player_is_landlord == 1) {
		gotoxy(Y_point + 20, MapHeight - 6);
		cout << "【地主】";
...
void dachu(int* a) {
	...
	Draw(40, tempLeft_y - 20 , MapLength - 80, cardHeight + 4);
...
void Ai_Left_Chu(int *ai_card_chu,int ai_card_num) {
	...
	for (int i = temp_left_x-1; i < temp_left_x + temp_card_length+1; i++) {
		for (int j = 8; j < MapHeight; j++) {
			Map[i][j] = '0';
			/*gotoxy(i, j);
			putchar(' ');*/
	...
	Draw(4, 8, temp_card_length + 2, MapHeight-10);
	gotoxy(13, temp_left_y + 1);
	cout << ai_card_num << "张";
...
void Ai_Right_Chu(int* ai_card_chu, int ai_card_num) {
	...
	gotoxy(MapLength - 32, MapHeight / 2);
	cout << "    ";
	...
	for (int i = temp_right_x-1; i < temp_right_x + temp_card_length+1; i++) {
		for (int j = 8; j < MapHeight; j++) {
			Map[i][j] = '0';
			/*gotoxy(i, j);
			putchar(' ');*/
	...
	Draw(temp_right_x - 1, 8, temp_card_length + 2, MapHeight - 10);
	gotoxy(MapLength - 13, temp_right_y + 1);
	cout << ai_card_num << "张";
